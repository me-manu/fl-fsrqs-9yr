...
We characterize long-term $\gamma$-ray variability and brightest $\gamma$-ray flares of six FSRQs.
...
we find variability on timescales on sub-hour scales and as short as minutes for  two sources in our sample (3C279, CTA102) and weak evidence for sub-orbital variability for PKS1510-089 and 3C454.3, which suggests extremely compact emission regions in the jet.
...
modulations in the accretion rate with the jet activity~\citep{2009A&A...503..797G}.


\begin{figure}
    \includegraphics[width = .99\linewidth]{figures/fluxdist_weekly_tsmin9.pdf}
    \caption{\label{fig:fluxpdf} Distribution of the fluxes of the weekly 9.5 year \gray light curves. The BPL fit is shown as a black line with $1\,\sigma$ uncertainties shown as shaded bands. The black dashed line indicates a fit with a log-normal distribution with location and scale parameters as indicated in the legends.}
\end{figure}
...
We includ all data points detected with $\mathrm{TS} \geqslant 9$ and perform a linear interpolation between gaps in the light curve to guarantee an even sampling. 
...
The interpolation is done in time steps of $\Delta t=0.7\,$days and 
 re-binned into bins of lengths of 7 days through averaging the fluxes.
...
\end{equation}
and calculate the $p$-value, $p_\beta$, as the  fraction of simulations that result in $\chi^2_\mathrm{sim}(\hat\beta,\hat\beta) > \hat\chi^2$.
...
\enddata
{
\tablecomments{Columns 2-4 indicate the best-fit values for the BPL fit [Eq.~(\ref{eq:dndf})] to the $dN/dF$ distributions, whereas columns 5-7 show the best-fit results for the PSD. $\beta_\mathrm{slope}$ gives the result for a linear regression of the periodograms and $\hat\beta$ is the best-fit value of the $\chi^2$ minimization with corresponding $p_\beta$-value. The interval around $\hat\beta$ is at 95\,\% confidence.}
}
\end{deluxetable*}
...
We proceed with deriving local properties of the \gray flares, focusing first on the light curves with one bin per GTI that are shown in Fig.~\ref{fig:gti}.
The average best-fit spectral parameters for the entire time spans of the daily, orbital, and sub-orbital light curves (the time ranges for the sub-orbital light curves are indicated as shaded bands in Fig.~\ref{fig:gti}) are summarized in Tab.~\ref{tab:avg-spec} in Appendix~\ref{sec:avg-spec}. 
...
\begin{eqnarray}
    ...
    \label{eq:flareHOP}
\end{eqnarray}
where $t_{0,ij}$ are the flare peak times, and $\tau_{\mathrm{rise},ij}$, $\tau_{\mathrm{decay},ij}$ are the flare rise and decay times, respectively.
...
The reason for allowing $N_i > 1$ is that the flare profile in Eq.~\ref{eq:flareHOP} does not capture long-lasting plateaus of a flare present in multiple flares in Fig.~\ref{fig:gti} (see, e.g., all flares of 3C\,279 or the last panel with the flare of 3C\,454.3 starting at 55551.65\,MJD).
...
In this way we ensure high photon statistics and reduce the number of trials when searching for minute scale variability (for comparison, the orbital light curve bin for which \citet{TheFermi-LAT:2016dss} measured minute scale variability is detected in our analysis with $\mathrm{TS} \sim 400$). 
...
In order to make an unbiased selection of GTIs that we test against the hypothesis of a constant flux, we consider only those where the BBs indicate a significant flux change within the GTI (GTIs marked in green in Fig.~\ref{fig:lc_minutes}).
...
For the GTIs where the constant fit results in a pre-trial $p$-value of less than 0.1, we report the The pre- and post-trial fit probabilities in Tab.~\ref{tab:minute}.
...
From the rich data set of the six FSRQs studied here, we attempt to constrain the position of the emitting region using two independent approaches. 
...
We discuss the impact on the results due to the simplifying assumptions made for this BLR model below.
...
\enddata
{
\tablecomments{Highest energy photons are given for source probabilities $> 0.99$. The decay times given for the flare component with highest peak flux as determined in the fit to the orbital light curves in Sec.~\ref{sec:results-local}.}
}
\end{deluxetable*}
...
The average values for $\delta_\mathrm{D}$ and $\Gamma$ obtained from Very Long Baseline Array monitoring observations are used \citep[see also Tab.~\ref{tab:src-select}]{2017ApJ...846...98J}  and the total  uncertainty is obtained by summing the uncertainties on $\delta_\mathrm{D}$, $\Gamma$, and the fit uncertainty of $t_\mathrm{var}$ in quadrature.
...
As we do not observe significant cut-offs in the spectra, we therefore conclude that the ring geometry adopted here provides a conservative limit on the distance of the emission region to the black hole. 
...
Additionally, \citet{2013ApJ...763L..36L} find that the BLR brightening coincides with the passage of a superluminal jet component through radio core, which could indicate that BLR clouds are located at larger distances, $\gtrsim 1\,$pc, then assumed here.
...
With the limits on $r$ it is now possible to derive constraints on the energy density of external photon fields in the co-moving frame, which could be responsible for the \gray emission due to inverse-Compton (IC) scattering with relativistic electrons in the emission region. 
...
The BLR cooling time for one flare of CTA102 is shown for $r=r_\mathrm{lim}$ and $r = 1\,$pc as a function of $\gamma'$ and $E_\mathrm{obs,BLR}$ in Fig.~\ref{fig:tcool}  (black solid and dash-dotted lines, respectively), assuming again the average values for $\delta_\mathrm{D}\sim31$ and $\Gamma\sim22$ derived from VLBA observations. 
...
We repeat the fits of the exponential profiles to the energy dependent light curves where proceed in the same way as in Sec.~\ref{sec:results-local} but allowing only one flare profile per HOP group. 
...
However, from the fits to the light curves, the decay time at higher energies is actually be longer ($0.45\pm0.16$ days above 1\,GeV versus $0.22 \pm 0.03$ between 0.1 and 0.3\,GeV). 
...
For the binning of the time lags $\tau$ we choose the half the maximum of the median of the time separations between consecutive data points in the two light curves.
...
The reason for this might be a correlation between uncertainties and flux, which is not taken into account by the adopted simulation scheme, and which can lead to an overestimation of the simulated uncertainties.
...
Having determined the best-fit PSDs, we them it to create artificial light curves in the same way as for fitting the periodogram itself.
...
We show the observed LCCF between \gray and radio light curves in Fig.~\ref{fig:lccf} for the cases where we find a significant peak in the LCCF. This is the case for 3C273 (OVRO and SMA), CTA102 (ALMA), and 3C454.3 (all radio light curves correlate with the \gray light curve).
...
In order to identify flaring episodes in an unbiased way, we have introduced a novel combination of Bayesian blocks (BBs)~\citep{2013ApJ...764..167S} and a hill-climbing algorithm inspired by the HOP group finding algorithm~\citep{1998ApJ...498..137E}, see Sec.~\ref{sec:zoom}.
...
In general, we find a large variety of flares showing both fast-rise and exponential decay (FRED) behaviour and the opposite. 
...
With our novel approach of identifying flares, it will be possible in the future to build large samples of flares whose properties could be compared in more detail to predictions of the reconnection scenario.
...
In order to reconcile cooling times with minute-scale variability, the emission region would have to be close to the found lower limits. 
...
At least at high electron energies, cooling with BLR photons occurs in the Klein-Nishina regime whereas cooling with dust-torus photons occurs in the Thompson regime (see Fig.~\ref{fig:tcool}).
...
If the distances are even larger, the radiative cooling with BLR photons becomes inefficient (and the cooling times will be suppressed with $\Gamma^2$) and cooling through IC scattering with dust-torus photons does not reproduce the observed flare decay times. 
...
\label{sec:avg-spec}
In Tab.~\ref{tab:avg-spec} we present the average best-fit observed spectra for the analyzed FSRQs for different time ranges. 
...
\enddata
{
\tablecomments{The light curves for the respective time intervals are shown in Figs.~\ref{fig:daily}, \ref{fig:gti}, and, for the cases where at least one orbital bin is detected with $\mathrm{TS}\geqslant150$, in Fig.~\ref{fig:lc_minutes}.
