...
As described in Sec.~\ref{sec:zoom}, we search for sub-orbital variability in the subset of orbital light curves. 
We choose only those light curves where at least one orbital light curve bin is detected with $\mathrm{TS} \geqslant 150$. 
This is motivated by the evidence for minute-scale variability in 3C279. For the bin where \citet{ackermann2015} found evidence for the short-time variability, we find \todo{$\mathrm{TS} = XXX$}. 
With our $\mathrm{TS}$ cut, we ensure similarly high photon statistics in reduce the number of trials when searching for minute scale variability. 
The selected time regions are indicated with the green shaded regions in Fig.~\ref{fig:gti}, whereas the blue shaded regions indicate the time intervals selected with the criteria in Tab.~\ref{tab:zoom} that do not pass the additional $\mathrm{TS}$ cut.
...
Naively, one would think that a BB change within one GTI would correspond to a significance of 95\,\% for a non-constant flux, since this is the threshold we have selected in the BB algorithm~\citep{Scargle2012}. However, the BBs take also data  before and after this GTI into account and only provide qualitative evidence for minute-scale variability. 
...
\todo{Comparison to previous results: lower significance, due to trial correction, different analysis: searched for additional point source (where do find them), different exposure due to binning in $\phi$ direction, different light curve binning (additonal binnings would increase number of trials). Taken $\chi^2$ test and BBs together, we find weak evidence for minute-scale variability for 3C454.3 and PKS1510-089 and stronger evidence for 3C279 and CTA102.}
\begin{figure*}
    \centering
...
%        where $\delta_\mathrm{D}$ is the Doppler boost, $\epsilon_\mathrm^{obs}_s$
    ...
    \item The cooling time in the observer's frame is then calculated to be 
    \begin{equation}
