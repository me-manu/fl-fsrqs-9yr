...
\begin{figure*}
    \centering
    \includegraphics[width = .99\linewidth]{figures/lc_daily_tsmin4.pdf}
...
further quantify it by calculating the flux distribution, $dN/dF$, of the weekly fluxes for bins with $\mathrm{TS} > 9$. 
...
The observed periodograms are calculated with Eq.~(3) of \citet{2014MNRAS.445..437M} including all 
data points detected with $\mathrm{TS} \geqslant 9$ and performing a linear interpolation between gaps in the light curve. Since we are using weekly binned light curves and bright FSRQs the gaps are small and at most 5 consecutive data points long (35\,days) in the case of 3C\,273. The number of non-detected bins is less than X\,\% for all sources.
...
The only exception is 3C\,279 where only 12 of the $10^4$ simulated light curves result in a $\chi^2_\mathrm{sim}(\hat\beta,\hat\beta) > \hat\chi^2$, which also explains the large error bars on the reconstructed value of $\beta$.
...
\begin{figure*}
    \centering
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_PKSB1222+216_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_3C273_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_3C279_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_PKS1510-089_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_CTA102_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \includegraphics[width = 0.32\linewidth]{figures/periodogram_fermi_3C454p3_Nsim_100Next_100Sim_addunc_data_rescale_EM13_usegap_1_PSD_window_none_detrend_none_norm_var.pdf}
    \caption{Periodograms of the observed and simulated light curves.}
...
\tablewidth{0pt}
\tablecaption{ \label{tab:global}Global \gray light curve properties. Columns 2-4 indicate the best-fit values for the BPL fit [Eq.~(\ref{eq:dndf})] to the $dN/dF$ distributions, whereas columns 5-7 show the best-fit results for the PSD. $\beta_\mathrm{slope}$ gives the result for a linear regression of the periodograms and $\hat\beta$ is the best-fit value of the $\chi^2$ minimazation with corresponding $p$-value. The interval around $\hat\beta$ is at 95\,\% confidence.}
...
\startdata
PKSB1222+216 & $-0.24^{+0.41}_{-0.27}$ & $-2.70^{+0.33}_{-0.43}$ & $0.42^{+0.28}_{0.15}$ & 1.13 & $1.15^{+0.17}_{-0.32}$ & 0.611 \\
3C273 & $0.70^{+0.40}_{-0.54}$ & $-2.77^{+0.24}_{-0.27}$ & $0.23^{+0.07}_{0.07}$  & 1.02 & $1.09^{+0.29}_{-0.38}$ & 0.403\\
3C279 & $0.68^{+0.27}_{-0.40}$ & $-2.80^{+0.21}_{-0.23}$ & $0.40^{+0.10}_{0.10}$ & 0.59 & $0.61^{+0.61}_{-0.55}$ & 0.001 \\
PKS1510-089 & $0.84^{+0.72}_{-0.48}$ & $-2.21^{+0.23}_{-0.26}$ & $0.40^{+0.16}_{0.12}$ & 0.76 & $0.97^{+0.31}_{-0.38}$ & 0.125\\
CTA102 & $-0.35^{+0.22}_{-0.18}$ & $-2.50^{+0.16}_{-0.19}$ & $0.90^{+0.38}_{0.26}$ & 1.13 & $1.18^{+0.36}_{-0.47}$ & 0.195\\
3C454.3 & $-0.20^{+0.17}_{-0.13}$ & $-3.00^{+0.13}_{-0.14}$ & $2.19^{+0.39}_{0.32}$ & 0.97 & $1.21^{+0.59}_{-0.52}$ & 0.228 \\
\enddata
\end{deluxetable*}
...
After each HOP shed is fitted individually and $N_i$ is determined for each shed, 
we re-fit the entire light curve, which consists of $N_\mathrm{HOP}$ sheds, with the function 
\begin{equation}
...
In general, the $\chi^2$ values divided by the degrees of freedom (dof) are between 1 and 2 (see the legends in Fig.~\ref{fig:gti}). Given the large values of dof, the fit qualities are usually poor. This is not unexpected, as we only allow up to two flare profiles per HOP and not arbitrary functions. Already with this chocice, there are probably some spurios flares identified, e.g., the second flare profile in the first PKS\,1510-089 flare (starting at 54908.65MJD). 
Nevertheless, the overall light curve evolution is well captured allowing us to describe the local flare properties from the ensembles of flare profiles, keeping the above caveats in mind. 
