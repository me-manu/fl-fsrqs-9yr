...
Surprisingly, at \gray energies flux doubling times as short as minutes have been observed both in BL Lac-type objects and flat spectrum radio quasars (FSRQs) with ground-based Cherenkov telescopes and the \FermiLAT~\cite[e.g.,][]{2007ApJ...669..862A,pks2155hess2007,pks1222magic2011,2013ApJ...762...92A,2014Sci...346.1080A,TheFermi-LAT:2016dss,2018ApJ...854L..26S}.
...
The identified time ranges for the weekly light curves are plotted as colored horizontal solid lines in Fig.~\ref{fig:weekly}.
The intermediate daily light curves are provided in Fig.~\ref{fig:daily}.
Figure~\ref{fig:gti} shows the GTI (equivalent to orbital) light curves with exponential fits of flare profiles that we will discuss in Sec.~\ref{sec:results-local}. 
...
\begin{figure*}
    ...
    \caption{\label{fig:daily} Light curves with daily binning for the selected time ranges (horizontal lines in Fig.~\ref{fig:weekly}). Symbols and lines are the same as in Fig.~\ref{fig:weekly}. 
...
\subsection{Determination of the quiescent background}
As mentioned in the introduction, 
there is no rigorous way to distinguish 
fluxes in flares from those 
in a quiescent background (hereafter QB).
...
but is a crude lower limit  -- biased downward 
...
\todo{Also mention $F_\mathrm{br}$ here}
...
and the intervening intervals are pure QB.
The algorithm attempts to implement this separation
entirely based on the flux values with no regard to 
...
\begin{enumerate}
\item Sort the $N$ flux values in increasing order:
 %$F_{n}, n = 1, 2, \dots 
$F_{1} \leqslant F_{2} \leqslant \dots F_{N-1}   \leqslant F_{N} $
\item Define a search grid of flux values, $\phi$, to find the peak of the distribution. We take flux values $m = 1,\ldots,M$ on an evenly spaced grid with $\phi_m \in [F_1 + \epsilon (\bar{F} - F_1); \bar{F}]$, 
where $\epsilon < 1$ is a factor to avoid edge effects on the lower flux end and $\bar{F}$ is the mean flux (we expect the QB to be lower than the mean flux).

\item Define two intervals  of equal length straddling  $\phi_m$: 
    \begin{itemize}
    \item LOW: from $F_{1}$ to $F_{n} < \phi_m$
    \item HIGH: from $F_{n}$ to $F_{n} + (F_{n} - F_{1}$ )
    \end{itemize}
...
\item Normalize both CDFs to unity at the peak $F_{n}$
...
\item Find the value of $\phi_m$ that minimizes this ratio, which measures the symmetry of the total CDF of HIGH and LOW
...
in the limit of infinitely finely gridded bins this is an
unbinned CDF estimate.  In practice a modestly
large number of small bins results that are
effectively unbinned, with essentially no dependence
on the binning.
We show our estimate for the QB level in Fig.~\ref{fig:weekly} as black dashed lines and report the values in Tab.~\ref{tab:global}.
\todo{maybe too high: contamination from flares; lower than $F_\mathrm{br}$, tested with simulations also using other metrics like KS test or least squares of CDF, found that our algorithm performed the most reliably, slightly overestimating the true QB level (try different flare types too). Can be seen as an upper limit of the QB if it is truely constant over observation time. Say that use of peak or median does not matter / does not change results much}
...
In general, the periodograms are well fit by our method, as indicated by the $p_\beta$-values and observed in Fig.~\ref{fig:periodograms}. 
...
The average best-fit spectral parameters for the entire time spans of the daily, orbital, and sub-orbital light curves (the time ranges for the sub-orbital light curves are indicated as solid horizontal bars in Fig.~\ref{fig:gti}) are summarized in Tab.~\ref{tab:avg-spec} in Appendix~\ref{sec:avg-spec}. 

%%% GTI light curves %%%%%%%%%
\begin{figure*}
    \centering
    \includegraphics[width = .99\linewidth]{figures/lcfithop_orbit_all_maxiter2_fsys0p00_addcomp0_comb.pdf}
    \caption{ Light curves with one bin per GTI for the selected time ranges (solid horizontal lines in Fig.~\ref{fig:daily}). Solid black and dashed lines show fits to the light curve with exponential flare profiles discussed in Sec.~\ref{sec:results-local}. Other symbols and lines are the same as in Fig.~\ref{fig:weekly} and ~\ref{fig:daily}.
    The sub-GTI light curves are derived for the time intervals indicated as solid horizontal lines, where at least one orbital bin shows $\mathrm{TS}\geqslant 150$ (see Sec.~\ref{sec:sub-gti}). 
    Average spectra to search for a cut-off due to \gray absorption in the BLR are derived from the time intervals indicated either as solid or dashed horizontal lines (see Sec.~\ref{sec:blrabs}).
    }
    \label{fig:gti}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    \includegraphics[width = .7\linewidth]{figures/lcfithop_results_tdtrhist_maxiter2_fsys0p00_addcomp0_orbit.pdf}
    ...
    \caption{
    %Stacked bar graphs for  rise (top) and decay times (bottom) for the individual flares fitted in Fig.~\ref{fig:gti}. The legend gives the horizon crossing time scales $t_g$ in the observer's frame.
...
The selected time regions are indicated with solid horizontal lines regions in Fig.~\ref{fig:gti}, whereas the dashed lines show the time intervals selected with the criteria in Tab.~\ref{tab:zoom} that do not pass the additional $\mathrm{TS}$ cut.
...
\enddata
{
\tablecomments{The number of trials is counted for each flare individually and given by the number of horizontal solid lines in each panel of Fig.~\ref{fig:lc_minutes}.}
}
\end{deluxetable}
...
First, in Sec.~\ref{sec:blrabs}, we search for absorption signatures in the LAT spectra caused by pair production of \Grays with photons of external photon fields. We use spectra during the brightest flares identified in the orbital light curves in Fig.~\ref{fig:gti} (dashed and solid horizontal bars).
...
\enddata
{
\tablecomments{Highest energy photons are given for source probabilities $> 0.99$. The decay times are given for the flare component with highest peak flux as determined in the fit to the orbital light curves in Sec.~\ref{sec:results-local}.
...
The decay times derived from the fit of the two bright flares of CTA102 around MJD 57738 are shown is dark green squares (see the first solid horizontal line in the panel in the 4th row and 2nd column in Fig.~\ref{fig:gti}), where one decay time is shown as an upper limit due to its large uncertainty. 
